...
class VideoAdapter : BaseRealmRecyclerViewAdapter<Video, VideoAdapter.VideoViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VideoViewHolder {
        val itemView: View = LayoutInflater.from(parent.context).inflate(R.layout.item_video, parent, false)
    ...
    inner class VideoViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun getClickSubject(item: Video): Observable<Video> {
            return Observable.create<Video> { emitter -> itemView.setOnClickListener { v -> emitter.onNext(item) } }
        }
